...
namespace TRANSCONNET
{
    ...
    public LectureFichier(Manager manager, string path)
    {
    ...
    // pos = 3 : adresse postale
    // pos = 4 : adresse mail
    // pos = 5 : téléphone
    // pos = 6 : date d'embauche (salarié )
    // pos = 7 : salaire (salarié )
    // pos = 8 : poste (salarié )
    // pos = 9 : Employeur : (salarié )
    // pos = 10 : Nom employeur (salarié )
    // pos = 11 : Prenom employeur (salarié )
    // pos = 12 : id (salarié )
    ...
    public void ReadFile()
    {
        ...
        string[] champsCSV = { "nom", "prenom", "date de naissance", "adresse postale", "adresse mail", "téléphone", "date d'embauche", "salaire", "poste", "Employeur :", "Nom", "Prenom", "id" };
        ...
        while (line != null && line != ";;;;;;;;;;;;") // le deuxième cas correspond à une ligne fantôme, créé par excel
        {
            ...
            if (mots[10] == "" || mots[10] == "TRANSCONNECT")
            {
                ...
                else manager.SalariesHierarchie = new ArbreSalarie(salarie);
            }
            else
            {
    ...
    public void Add(Salarie s, string nom, string prenom)
    {
        ...
        sr.Write(s.Nom + sep + s.Prenom + sep + birth + sep + s.AdressePostale + sep + s.AdresseMail + sep + s.Telephone + sep + embauche + sep + s.Salaire + sep + s.Poste + sep + "Employeur :" + sep + nom + sep + prenom + sep + s.Id + "\n");
        sr.Close();
    }
    ...
    {//ajoute à la première ligne
        ...
        string birth = s.DateNaissance.Day.ToString() + "/" + s.DateNaissance.Month.ToString() + "/" + s.DateNaissance.Year.ToString();
        string embauche = s.DateEmbauche.Day.ToString() + "/" + s.DateEmbauche.Month.ToString() + "/" + s.DateEmbauche.Year.ToString();
        tr.Write(s.Nom + sep + s.Prenom + sep + birth + sep + s.AdressePostale + sep + s.AdresseMail + sep + s.Telephone + sep + embauche + sep + s.Salaire + sep + s.Poste + sep + "Employeur :" + sep + nom + sep + prenom + sep + s.Id + "\n");
        line = sr.ReadLine();
        while (line != null)
        {
            tr.Write(line + "\n");
    ...
    public void Add(Client c)
    {
        ...
        sr.Write(c.Nom + sep + c.Prenom + sep + birth + sep + c.AdressePostale + sep + c.AdresseMail + sep + c.Telephone + sep + c.Id + "\n");
        sr.Close();
    }
    ...
    public void ReadFileCommandeArchived() // lis dans le commande.csv les commandes ayant déjà été passé par les clients, et leur associe
    {
        /*-------------------------------------------------------------------------------------------------------
